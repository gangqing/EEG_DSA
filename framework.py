...
def get_gpus():
    value = os.getenv('CUDA_VISIBLE_DEVICES', '1')  # 获取可使用的gpu，默认值为0
...
class App:
    ...
    def train(self, ds_train, ds_validation):
        ...
        for epoch in range(cfg.epoches):
            ...
            for batch in range(batches):
                ...
                print("epoch = {epoch} , batch = {batch} , loss = {losses}".format(epoch=epoch, batch=batch, losses=loss))
    ...
    def get_feed_dict(self, ds):
        ...
        for i in range(self.config.gpus):
            inputs = self.ts.sub_ts[i].inputs
